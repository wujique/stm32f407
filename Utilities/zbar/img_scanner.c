                               ...
                               int datalen)
{
    ...
    if(sym) {
        ...
        sym = (zbar_symbol_t*)zbar_calloc(1, sizeof(zbar_symbol_t));
        STAT(sym_new);
    }
    ...
    if(datalen > 0) {
        ...
        if(sym->data_alloc < datalen) {
            ...
            sym->data = (char *)zbar_malloc(datalen);
        }
    }
    else {
        if(sym->data)
...
{
    zbar_image_scanner_t *iscn = (zbar_image_scanner_t *)zbar_calloc(1, sizeof(zbar_image_scanner_t));
    if(!iscn)
        return(NULL);
