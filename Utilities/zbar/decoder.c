...
{
    zbar_decoder_t *dcode = (zbar_decoder_t *)zbar_calloc(1, sizeof(zbar_decoder_t));
    dcode->buf_alloc = BUFFER_MIN;
    dcode->buf = (unsigned char *)zbar_malloc(dcode->buf_alloc);
                                    ...
                                    unsigned int buflen)
{
    ...
    if(!decoder_dump || dumplen > decoder_dumplen) {
        ...
        decoder_dump = (char *)zbar_malloc(dumplen);
