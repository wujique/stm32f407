...
static int vfw_probe (zbar_video_t *vdo)
{
    ...
    BITMAPINFOHEADER *bih = state->bih = zbar_realloc(state->bih, state->bi_size);
    /* FIXME check OOM */
    ...
    vdo->formats = zbar_calloc(VFW_NUM_FORMATS, sizeof(uint32_t));
    ...
    vdo->formats = zbar_realloc(vdo->formats, (n + 1) * sizeof(uint32_t));
                      ...
                      const char *dev)
{
    video_state_t *state = vdo->state;
    if(!state)
        state = vdo->state = zbar_calloc(1, sizeof(video_state_t));
