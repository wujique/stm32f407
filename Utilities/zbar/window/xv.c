...
static int xv_cleanup (zbar_window_t *w)
{
    ...
    zbar_free(x->xv_ports);
    zbar_free(x->xv_adaptors);
    x->xv_ports = NULL;
    x->num_xv_adaptors = 0;
    x->xv_adaptors = NULL;
    return(0);
                                 ...
                                 XvPortID port)
{
    ...
    window_state_t *x = w->state;
    if(!w->formats[i + 1])
        x->xv_ports = zbar_realloc(x->xv_ports, (i + 1) * sizeof(*x->xv_ports));
...
int _zbar_window_probe_xv (zbar_window_t *w)
{
    ...
    window_state_t *x = w->state;
    x->num_xv_adaptors = 0;
    x->xv_adaptors = zbar_calloc(n, sizeof(*x->xv_adaptors));
    int i;
    for(i = 0; i < n; i++) {
    ...
    if(x->num_xv_adaptors < n)
        x->xv_adaptors = zbar_realloc(x->xv_adaptors,
                                 x->num_xv_adaptors * sizeof(int));

    w->max_width = w->max_height = 65536;
    w->formats = zbar_realloc(w->formats, sizeof(uint32_t));
    w->formats[0] = 0;
    for(i = 0; i < x->num_xv_adaptors; i++)
